...
export function ContestCard(contest: Contest) {
  ...
  const joinContest = async () => {
    ...
    await axios.post(`http://localhost/api/contests/${contest.id}/join`, null,
      {
        headers: {
  ...
  const unjoinContest = async () => {
    const token = localStorage.getItem("token")
    await axios.delete(`http://localhost/api/contests/${contest.id}/unjoin`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  useEffect(() => {
    const getJoinContestStatus = async () => {
      const token = localStorage.getItem("token")
      await axios.get(`http://localhost/api/contests/${contest.id}/join-status`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
